...
public class CsvSPDE {
    ...
    static String[] HEADERs = {"Bailleur", "Type de coopération", "Type de fond","Montant A Rembourser",  "Interet","Total", "Echeance", "Date"};
    ...
    public static ByteArrayInputStream writeSend(List<ServPreDetteExt> servPreDetteExts) {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
            ...
            for (ServPreDetteExt servPreDetteExt : servPreDetteExts) {
                ...
                if (servPreDetteExt.getTotal() != null) {
                    row.createCell(5).setCellValue(servPreDetteExt.getTotal());
                }
                if (servPreDetteExt.getEcheance() != null) {
                    row.createCell(6).setCellValue(servPreDetteExt.getEcheance());
                }
                if (servPreDetteExt.getDate() != null) {
                    row.createCell(7).setCellValue(servPreDetteExt.getDate());
                }
                ...
                while (cellsInRow.hasNext()) {
                    ...
                    switch (cellIdx) {
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            servPreDetteExt.setMontant_a_rembourser(currentCell.getColumnIndex());
                            break;
                        case 4:
                            servPreDetteExt.setInteret(currentCell.getColumnIndex());
                            break;
                        case 5:
                            servPreDetteExt.setTotal(currentCell.getColumnIndex());
                            break;
                        case 6:
